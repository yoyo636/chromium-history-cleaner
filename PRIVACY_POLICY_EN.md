...
- **Network requests initiated by the extension itself** are limited to one type, executed by the browser: the bookmarks "dead-link check" sends a `HEAD` request to a bookmark URL (transmitting only that URL itself). No other network transmission is made by the extension.
...
| `host_permissions` (http/https) | Privacy module script injection (fingerprint monitoring & randomization); eye-care / performance / ad-block content scripts run on web pages |
